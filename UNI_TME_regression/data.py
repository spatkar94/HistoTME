...
def build_mil_loader(args, df, subset, bag_size, task_counts):
    ...
    '''
        ...
        #sample_weights = [1/response_counts[i] for i in df.response_label.values]
        suny_counts = df.suny.value_counts()
        sample_weights = [sample_weights[i]/suny_counts[value] for i,value in enumerate(df.suny.values) ]
        sampler = WeightedRandomSampler(weights=sample_weights, num_samples=len(df), replacement=True)
        #sampler = WeightedRandomSampler(weights=sample_weights, num_samples=int(2*len(df)), replacement=True)
        loader = DataLoader(dataset(df, task_counts, bag_size), sampler=sampler,
